...
function BigAuras:GetOrCreate(unit)
	...
	if db ~= nil and db.enable then
		...
		if string.find(unit, "party") and GetCVar("useCompactPartyFrames") then
			parent = UIParent
		end
...
function BigAuras:UpateUnit(unit)
	if (self.db[unit] and self.frames[unit]) then
		...
		for _, auraFilter in ipairs(auraFilters) do
			for i = 1, 40 do
				local arg1, arg2, arg3, arg4, duration, expirationTime, arg5, arg6, arg7, spellID = UnitAura(unit, i, auraFilter)
				if spellID ~= nil then
